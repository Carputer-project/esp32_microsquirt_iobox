...
static constexpr uint8_t  RESP_PORT_FAN_BIT = 0x80;
...
static constexpr uint16_t CFG_MAGIC = 0x4963;
...
static void handleCommand(const String& line) {
    ...
    switch (key) {
        ...
        case 'P': {
            ...
            if (val.length() == 0) {
                Serial.printf("pins iac=%u o1=%u o2=%u o3=%u o4=%u o5=%u o6=%u o7=%u\n",
        ...
        default:
            Serial.println("commands: ? | P[IAC <pin>|O<n> <pin>|TFT 0|1|RESET] | F[onTempF|A|1|0] | E[offTempF] | I[duty|A|F] | T[targetRpm] | Y[fanOut 1-7|0] | S[shiftRpm] | O<n>[0|1|T<f>|R<rpm>] | A<n>[0|O<k> <v>|F <v>|<v>] | R[0|1|B<id>] | W[0|1|idle|maxrpm|clt|mat|batt|map|afr|hold|warnout|help]");
            break;
    }
}
...
static bool pinOk(uint8_t p) {
    ...
    if (p == 4 || p == 5) return false;                     // CAN TX/RX
...
void setup() {
    ...
    Serial.println("MS2/Extra I/O box v3 (iobox3) ready. Type ? for status.");
}
