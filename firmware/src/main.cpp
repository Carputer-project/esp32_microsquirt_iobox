...
static constexpr uint16_t CFG_MAGIC = 0x4958;
...
static void updateOutputs() {
    ...
    bool fanOn = false;
    if (respActive) {
        fanOn = (s_respPort[0] & RESP_PORT_FAN_BIT) != 0;
    ...
    uint8_t duty;
    if (respActive) {
        duty = (uint8_t)constrain((int16_t)(s_respPort[1] * 100 / 255), 0, 100);
...
static void handleCommand(const String& line) {
    ...
    switch (key) {
        ...
        default:
            Serial.println("commands: ? | F[onTempF|A|1|0] | E[offTempF] | I[duty|A|F] | T[targetRpm] | Y[fanOut 1-6|0] | S[shiftRpm] | O<n>[0|1|T<f>|R<rpm>] | D<n>[0|F|O<k>] | A<n>[0|O<k> <v>|F <v>|<v>] | R[0|1|B<id>]");
            break;
    }
}
